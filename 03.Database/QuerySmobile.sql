...
FROM PRODUCT AS P 
		JOIN (SELECT DISTINCT PC.* FROM PRODUCT_COMMENT AS PC JOIN PRODUCT AS P ON PC.PRODUCT_ID = P.PRODUCT_ID WHERE PC.PRODUCT_ID = 1) AS PM ON RP.PRODUCT_ID = PM.PRODUCT_ID
		JOIN USER AS U ON PM.USER_ID = U.USER_ID 
		JOIN (SELECT DISTINCT R.USER_ID, R.RATE_NUMBER FROM RATE_PRODUCT AS R JOIN PRODUCT AS P ON R.PRODUCT_ID = P.PRODUCT_ID WHERE R.PRODUCT_ID = 1) 
...
FROM PRODUCT_COMMENT AS PM 
	 JOIN RATE_PRODUCT AS RP ON (RP.USER_ID = PM.USER_ID AND RP.PRODUCT_ID = PM.PRODUCT_ID)
	 JOIN USER AS U ON (RP.USER_ID = U.USER_ID AND PM.USER_ID = U.USER_ID)
